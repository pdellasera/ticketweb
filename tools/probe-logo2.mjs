...
async function render(name, S) {
  ...
  for (let y = 0; y + S <= height; y += S) {
    ...
    for (let x = 0; x + S <= width; x += S) {
      ...
      const n = S * S
      if (opaque < n * 0.12) line += ' '
      else if (white > n * 0.28) line += 'W'
      else if (blue > n * 0.28) line += 'B'
      else if (dark > n * 0.28) line += '#'
      else line += 'o'
    }
    rows.push(line)
  }
...
await render('logo_white.webp', 5)
await render('logo.png', 24)
